...
from certicat import Certicat, CerticatResult
...
def test_pem():
    ...
    pem_cert = Certicat(certificate=pem_cert)
    pem_new_cert = pem_cert.get_copy()
    print(
        pem_new_cert.key.private_bytes(
            encoding=Encoding.PEM,
            format=serialization.PrivateFormat.TraditionalOpenSSL,
            encryption_algorithm=serialization.BestAvailableEncryption(b"radware"),
        ).decode("utf-8")
    )
...
def test_der():
    der_data = (
        b"0\x82\x05\x010\x82\x03\xe9\xa0\x03\x02\x01\x02\x02\x12\x03\t\x96\xf3O\x1b\x12\x12\xc8\xc1\xb8E\x06Uz"
    ...
    der_ccert = Certicat(certificate=der_cert)
    der_new_cert = der_ccert.get_copy()
    print(
        der_new_cert.key.private_bytes(
            encoding=Encoding.PEM,
            format=serialization.PrivateFormat.TraditionalOpenSSL,
